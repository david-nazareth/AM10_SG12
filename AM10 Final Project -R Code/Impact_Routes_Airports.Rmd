...
vis1 <- airports %>%
  ...
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"), strip.text.x = element_text(size = 12, face = "bold"), title =element_text(size=14, face='bold'), 
...
vis4 <- route_domestic %>%
  ...
  geom_hline(yintercept=0, color="orange") +
  theme_minimal() +
  labs(title= "Percentage change of flight volume each months in \n2019 and 2020", subtitle = "Comparison among 5 domestic routes", x= "", y="") +
  scale_x_discrete(labels = c("Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sep", "Oct")) +
...
vis2 <- route %>%
  ggplot() +
   geom_col(aes(x=(year), y=as.numeric(value)), width = 0.5, fill="#000088") +
  facet_wrap(~route, ncol=2, labeller = labeller(route = c("SBGR-EGLL" = "Sao Paulo - London", "EGLL-SBGR" = "London - Sao Paulo","EDDF-EGLL" = "Frankfurt - London", "EGLL-EDDF" = "London - Frankfurt", "KJFK-EGLL" = "New York - London","EGLL-KJFK" = "London - New York", "VIDP-EGLL" = "New Dehli - London", "EGLL-VIDP" = "London - New Dehli" ))) +
  geom_hline(yintercept=0, color="orange") +
  theme_minimal() +
  labs(title= "Percentage change in flight volume between 2020 and 2019, \nby month", subtitle = "Comparison among 5 international routes", x= "", y="") +
  scale_x_discrete(labels = c("Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sep", "Oct")) +
   theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +
   scale_y_continuous(labels = scales::percent) +
  theme(plot.margin = unit(c(1, 1, 0, 0), "cm"), strip.text.x = element_text(size = 12, face = "bold"), title =element_text(size=14, face='bold'), 
...
#change color to color scheme
route_test <- route_data %>%
filter(route=="SBGR-EGLL" | route=="EGLL-SBGR" )

vis2 <- route_test %>%
  ggplot() +
   geom_col(aes(x=(year), y=as.numeric(value), width = 0.5, fill=(as.factor(route))), position = position_dodge()) +
   scale_fill_manual(values = c("#000088", "lightblue"), name="", label= c("Sao Paulo - London", "London - Sao Paulo")) +
  geom_hline(yintercept=0, color="orange") +
  theme_minimal() +
  labs( x= "", y="") +
  scale_x_discrete(labels = c("Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sep", "Oct")) +
...
vis3 <- route_test3 %>%
  ...
  scale_fill_manual(values = c("#000088", "lightblue"), name="", label= c("Frankfurt - London", "London - Frankfurt")) +
  geom_hline(yintercept=0, color="orange") +
  theme_minimal() +
  labs( x= "", y="") +
  scale_x_discrete(labels = c("Jan", "Feb", "March", "April", "May", "June", "July", "Aug", "Sep", "Oct")) +
...
#look at David´s busiest routes
