---
title: "Impact of COVID-19 on Air Travel"
...
animate(p, nframes = 80, fps=2, height = 800, width =1200)
...
p <- p+ transition_time(day) +
  labs(title = "Date: {frame_time, '%b %d, %Y'}")

animate(p, nframes = 300, fps=3)

p

```
# Impact on vertical flight efficiency and additional taxi time
...
flight_efficiency_plot <- flight_efficiency_desc %>% 
  ...
  summarise(avg_time_level_desc= mean(avg_time_in_level_flight_seconds,na.rm=TRUE),
            ...
            lower_bound = round((avg_time_level_desc - t_critical * se_time_level_desc),digits=2),
            upper_bound = round((avg_time_level_desc+ t_critical * se_time_level_desc), digits=2)
  ) %>% 
  mutate(is_2020= as.factor(ifelse(year==2020,1,0)))


ggplot(flight_efficiency_plot, aes(x=year, y=avg_time_level_desc, colour=is_2020)) +
  geom_point(size=3) +
  geom_errorbar(width=.5, aes(ymin=lower_bound, ymax=upper_bound),  size= 1) + 
  labs( y= "Mean Time Level During CDO (seconds)", 
       title="Vertical flight efficiency during CDO was significantly lower in 2020") + 
  theme_minimal()+
  coord_flip()+
   theme(
    axis.title.y = element_blank(),
...
ggplot(flight_efficiency_plot, aes(x=year, y=avg_time_level_asc, colour=is_2020)) +
  geom_point(size=3) +
  geom_errorbar(width=.5, aes(ymin=lower_bound, ymax=upper_bound),  size= 1) + 
  labs( y= "Mean Time Level During CCO (seconds)", 
       title="Vertical flight efficiency during CCO was significantly lower in 2020") + 
  theme_minimal()+
  coord_flip()+
   theme(
    axis.title.y = element_blank(),
...
ggplot(taxi_out_time_plot, aes(x=year, y=avg_add_taxi, colour=is_2020)) +
  ...
  labs( y= "Mean additional taxi-out time (minutes)", 
       title="Average additional taxi-out time is significantly lower in 2020") + 
  theme_minimal()+
  coord_flip()+
   theme(
    axis.title.y = element_blank(),
